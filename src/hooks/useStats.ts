...
export const useStats = (activities: Array<Activity>) => {
    ...
    const calculateDaysInBetweenDates = useCallback((dateString1: string, dateString2: string) => {
    ...
    const calculatePercentageRatio = useCallback((metric: number, totalAccountable: number) => {
    ...
    const calculateSummary = useCallback((activities: Array<Activity>) => {
        ...
        const daysInBetween = calculateDaysInBetweenDates(activities[activities.length -1].date, activities[0].date);
        return {
